...
async fn get_item_history(path: web::Path<i64>, data: web::Data<AppState>) -> impl Responder {
    ...
    match cached {
        Ok(v) => {
            let tick: serde_json::Value = serde_json::from_str(&v).unwrap();
            HttpResponse::Ok().json(json!({"source": "redis", "data": tick}))
        },
        Err(_) => {
            // Fetch from MinIO using DuckDB
            let result = actix_web::web::block(move || {
                ...
                let query = format!(
                    "SELECT timestamp, avg_high_price, high_price_volume, avg_low_price, low_price_volume 
                     ...
                     WHERE item_id = {} ORDER BY timestamp ASC",
                    item_id
                );
