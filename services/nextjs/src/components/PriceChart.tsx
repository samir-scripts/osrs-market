...
export default function PriceChart({ itemId, itemName }: { itemId: number; itemName: string }) {
  ...
  const fetchHistory = async () => {
    try {
      setLoading(true);
      const res = await fetch(`/api/analytics/history/${itemId}?days=${daysframe}`);
  ...
  return (
    <Panel
      title={`${itemName.toUpperCase()} (ID: ${itemId}) - HISTORICAL PRICE & VOLUME`}
      headerRight={
    ...
    >
      {loading && <div style={{ padding: '24px', color: 'var(--color-text-muted)' }}>LOADING HISTORICAL TICK DATA FROM DUCKDB / MINIO...</div>}
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={data} margin={{ top: 10, right: 10, left: 10, bottom: 20 }}>
              ...
              <YAxis
                ...
                tickFormatter={(tick) => `${tick.toLocaleString()} gp`}
              />
              <YAxis
                yAxisId="volume"
